...
namespace dae
{
	struct Camera
	{
		...
		const float rotationSpeed{ 360 * TO_RADIANS };
		...
		void Update(const Timer* pTimer)
		{
			...
			const float mouseMovementScaler{ 10.0f };
			...
			if ((SDL_BUTTON(1) & mouseState) != 0 && (SDL_BUTTON(3) & mouseState) != 0) {
				if (mouseY != 0) {
					origin -= mouseMovementScaler * pTimer->GetElapsed() * movementSpeed * up * (float(mouseY) / abs(mouseY));
				}
			}
				...
				if (mouseY != 0) {
					origin -= mouseMovementScaler * pTimer->GetElapsed() * movementSpeed * forward * (float(mouseY) / abs(mouseY));
				}
			}
