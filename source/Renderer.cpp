...
namespace dae {
		...
		m_pWindow(pWindow)
	{
		...
		// Init Camera
		m_Camera.Initialize(45.f, { 0.0f,0.0f,-50.0f }, float(m_Width) / m_Height);
		...
		m_pVehicleMesh = new Mesh<VertexUV>(m_pDevice, vertices, indices);
	...
	void Renderer::RenderSoftware(){
		...
		m_pVehicleMesh->TransformVerts(m_Camera);

		// Loop over triangles
		for (int triangleIndex{ 0 }; triangleIndex + 2 < m_pVehicleMesh->m_Indices.size(); ++triangleIndex) {

			// Get correct indexes based on the mesh's topology
			int index0{}, index1{}, index2{};
			if (m_pVehicleMesh->primitiveTopology == PrimitiveTopology::TriangleList) {

				index0 = m_pVehicleMesh->m_Indices[triangleIndex + 0];
				index1 = m_pVehicleMesh->m_Indices[triangleIndex + 1];
				index2 = m_pVehicleMesh->m_Indices[triangleIndex + 2];
				triangleIndex += 2;
			}

			if (m_pVehicleMesh->primitiveTopology == PrimitiveTopology::TriangleStrip) {
				...
				if (triangleIndex % 2 == 0) {
					...
					index1 = m_pVehicleMesh->m_Indices[triangleIndex + 2];
					index2 = m_pVehicleMesh->m_Indices[triangleIndex + 1];
				}
			...
			// Render triangle
			Vertex_Out v0 = m_pVehicleMesh->vertices_out[index0];
			Vertex_Out v1 = m_pVehicleMesh->vertices_out[index1];
			Vertex_Out v2 = m_pVehicleMesh->vertices_out[index2];
				...
				for (int py{ pMin.y }; py <= pMax.y; ++py) {
					...
					if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
						...
						if (depthTestPassed) {
							...
							ColorRGB finalColor{ m_pVehicleMesh->PixelShading(pixelVertex) };
